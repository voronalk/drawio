...
(function () {
  ...
  mxUtils.getSizeForString = function (text, fontSize, fontFamily, textWidth) {
    return getSizeForString(
      Graph.sanitizeHtml(text),
      fontSize,
      fontFamily,
      textWidth
    );
...
Graph = function (
            ...
            if (
              ...
              if (handler == null || handler.getHandleForEvent(me) == null) {
                ...
                while (
                  ...
                  if (
                    this.isSwimlane(current.cell) &&
                    this.isCellMovable(current.cell)
                  ) {
                ...
                if (
                  ...
                  if (!this.isCellSelected(table)) {
                    if (
                      (mxUtils.intersects(
                        box,
                        new mxRectangle(state.x, state.y - 2, state.width, 4)
                      ) &&
                        this.model.getChildAt(table, 0) != row) ||
                      mxUtils.intersects(
                        box,
                      ...
                      (mxUtils.intersects(
                        box,
                        new mxRectangle(state.x - 2, state.y, 4, state.height)
                      ) &&
                        this.model.getChildAt(row, 0) != state.cell) ||
                      mxUtils.intersects(
                        box,
  ...
  if (typeof mxVertexHandler !== "undefined") {
    ...
    this.connectionHandler.insertEdge = function (
      parent,
      id,
      value,
      source,
      ...
      var edge = mxConnectionHandler.prototype.insertEdge.apply(
        this,
        arguments
      );
    ...
    this.click = function (me) {
      var locked =
        me.state == null &&
        me.sourceState != null &&
        this.isCellLocked(this.getLayerForCell(me.sourceState.cell));
    ...
    this.updateMouseEvent = function (me) {
      ...
      if (
        me.state != null &&
        this.isCellLocked(this.getLayerForCell(me.getCell()))
      ) {
          ...
          me.state = this.view.getState(
            this.getCellAt(
              ...
              mxUtils.bind(this, function (state, x, y) {
                return (
                  me.state == state ||
                  (this.isCellLocked(this.getLayerForCell(state.cell)) &&
    ...
    this.isCellSelectable = function (cell) {
      return (
        graphIsCellSelectable.apply(this, arguments) &&
    ...
    this.isCellLocked = function (cell) {
      while (cell != null) {
        if (
          mxUtils.getValue(this.getCurrentCellStyle(cell), "locked", "0") == "1"
        ) {
          return true;
        }
...
Graph.createSvgDarkModeCss = function (cssClass) {
  ...
  return (
    "svg" +
    cssClass +
    ":not(mjx-container > svg) { filter: invert(100%) hue-rotate(180deg); }\n" +
    "svg" +
    cssClass +
    " foreignObject img,\n" +
    "svg" +
...
Graph.htmlToPng = function (html, w, h, fn, css, scale) {
  ...
  var svg =
    ...
    '">' +
    (css != ""
      ? '<defs xmlns="http://www.w3.org/2000/svg"><style type="text/css">' +
        css +
        "</style></defs>"
      : "") +
...
Graph.arrayBufferIndexOfString = function (uint8Array, str, start) {
  ...
  for (
    var i = p + 1;
    p > -1 && i < uint8Array.byteLength && i < p + str.length - 1;
    i++
  ) {
...
Graph.exploreFromCell = function (sourceGraph, selectionCell, config) {
  ...
  function exploreFromHere(sourceGraph, selectionCell) {
          ...
          if (!ignored) {
            ...
            mxEffects.animateChanges(graph, changes, function () {
              ...
              layout.layout = function (parallels) {
                layout.spacing = Math.min(
                  60,
                  Math.max(30, 60 - parallels.length * 5)
                );
      ...
      if (graph.getModel().isVertex(cell)) {
        ...
        try {
          ...
          for (var key in graph.getModel().cells) {
            ...
            if (
              tmp != graph.rootCell &&
              !graph.getModel().isAncestor(graph.rootCell, tmp) &&
          ...
          var r = Math.max(
            minSize,
            Math.min(
              graph.container.scrollWidth / 2.5 - 80,
    ...
    function rootChanged(graph, cell) {
      ...
      var edges = sourceGraph.getEdges(
        sourceCell,
        null,
        true,
        true,
      ...
      if (
        cell.startIndex != null ||
        (pageSize > 0 && edges.length > pageSize)
      ) {
        var start = cell.startIndex || 0;

        cells = edges.slice(
          Math.max(0, start),
          Math.min(edges.length, start + pageSize)
        );
      }
      ...
      var edgeStyle =
        ";curved=1;noEdgeStyle=1;entryX=none;entryY=none;exitX=none;exitY=none;labelBackgroundColor=#ffffffc0;textOpacity=100;";
      var btnStyle =
        "fillColor=green;fontColor=white;strokeColor=green;rounded=1;";
          ...
          for (var j = 0; j < clones[i].getChildCount(); j++) {
            ...
            if (child.geometry != null) {
              if (child.geometry.relative) {
                child.setStyle(
                  child.getStyle() +
                    ";labelBackgroundColor=#ffffffc0;textOpacity=100;"
                );
      ...
      if (elt.nodeName == "P" && elt.className == "p1") {
        ...
        if (next != null && next.nodeName == "P" && next.className == "p1") {
          elt.parentNode.insertBefore(
            elt.ownerDocument.createElement("br"),
            elt
          );
        }
...
DOMPurify.addHook("afterSanitizeAttributes", function (node) {
  if (
    node.nodeName == "use" &&
    ((node.getAttribute("xlink:href") != null &&
    ...
    try {
      ...
      if (idx >= 0) {
        ...
        if (svgs.length > 0) {
          ...
          if (
            ignorePreserveAspect ||
            svgs[0].getAttribute("preserveAspectRatio") != null
          ) {
            ...
            try {
              ...
              if (size.width > 0 && size.height > 0) {
                ...
                if (
                  ...
                  div
                    .getElementsByTagName("svg")[0]
                    .setAttribute("width", size.width / fx);
                  div
                    .getElementsByTagName("svg")[0]
                    .setAttribute("height", size.height / fy);
                }
              }
            } catch (e) {
              // ignore
 ...
 */
Graph.prototype.placeholderPattern = new RegExp(
  "%(date{.*}|[^%^{^}^ ^\"^ '^=^;]+)%",
  "g"
);
...
(function () {
    ...
    if (temp != null) {
      ...
      try {
        ...
        for (var key in styles) {
          ...
          if (
            key == "edgeStyle" &&
            styles[key] == "elbowEdgeStyle" &&
            dir != null
          ) {
            this.setCellStyles(
              "elbow",
              dir == mxConstants.DIRECTION_SOUTH ||
                dir == mxConstants.DIRECTION_NOTH
                ? "vertical"
                : "horizontal",
              edges
            );
  ...
  Graph.prototype.getStartEditingCell = function (cell, trigger) {
    ...
    var size = parseInt(
      mxUtils.getValue(style, mxConstants.STYLE_STARTSIZE, 0)
    );
   ...
   */
  Graph.prototype.copyCellStyles = function (
    cells,
    keys,
    values,
    vertexStyle,
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (
        ...
        if (
          mxUtils.indexOf(Graph.cellStyles, keys[i]) >= 0 ||
          keys[i] == "shape"
        ) {
   ...
   *
   */
  Graph.prototype.pasteCellStyles = function (
    cells,
    vertexStyle,
    edgeStyle,
    force,
    ...
    pasteEdgeStyle =
      pasteEdgeStyle != null ? pasteEdgeStyle : this.pasteEdgeStyle;
      ...
      for (var i = 0; i < cells.length; i++) {
        ...
        if (Editor.simpleLabels) {
          cellStyle = mxUtils.setStyle(
            mxUtils.setStyle(cellStyle, "html", null),
            "whiteSpace",
            null
          );
        }
    ...
    try {
      for (var i = 0; i < cells.length; i++) {
        if (this.model.isVertex(cells[i]) || this.model.isEdge(cells[i])) {
          ...
          if (restorePerimeter) {
            this.setCellStyles(mxConstants.STYLE_PERIMETER, perimeter, [
              cells[i],
            ]);
          }
        }
    ...
    if (temp != null) {
      ...
      if (!this.useCssTransforms) {
        ...
        if (prev != g.getAttribute("transform")) {
          this.fireEvent(
            new mxEventObject("cssTransformChanged"),
            "transform",
      ...
      if (target == "_self" && window != window.top) {
        ...
        if (
          ...
          result = window.open(
            ...
            !allowOpener ? "noopener,noreferrer" : null
          );
...
Graph.prototype.initLayoutManager = function () {
  ...
  this.layoutManager.getLayout = function (cell, eventName) {
    ...
    if (
      ...
      (eventName != mxEvent.BEGIN_UPDATE || this.hasLayout(parent, eventName))
    ) {
      ...
      if (style["childLayout"] == "stackLayout") {
        ...
        if (stackLayout.allowGaps) {
          stackLayout.gridSize = parseFloat(
            mxUtils.getValue(style, "stackUnitSize", 20)
          );
        }
        ...
        style["childLayout"] != null &&
        String(style["childLayout"]).charAt(0) == "["
      ) {
        try {
          return new mxCompositeLayout(
            this.graph,
            this.graph.createLayouts(JSON.parse(style["childLayout"]))
          );
        } catch (e) {
...
Graph.prototype.createLayouts = function (list) {
  ...
  for (var i = 0; i < list.length; i++) {
    if (mxUtils.indexOf(Graph.layoutNames, list[i].layout) >= 0) {
      ...
      var layout =
        list[i].layout == "mxOrgChartLayout" && list[i].config != null
      ...
      if (list[i].config != null) {
        for (var key in list[i].config) {
          ...
          if (
            list[i].layout != "mxOrgChartLayout" ||
            key != "branchOptimizer"
          ) {
      ...
      if (state.shape != null) {
        ...
        for (var i = 1; i < n; i++) {
          if (dist <= segs[i - 1]) {
            pts2.push(
              new mxPoint(
                pts[i - 1].x + ((pts[i].x - pts[i - 1].x) * dist) / segs[i - 1],
...
Graph.prototype.getPageLayout = function (bounds, tr, s) {
  ...
  if (bounds.width == 0 || bounds.height == 0) {
    ...
    var w0 =
      Math.ceil(
        (Math.floor((bounds.x + bounds.width) / s) - tr.x) / size.width
      ) - x0;
    var h0 =
      Math.ceil(
        (Math.floor((bounds.y + bounds.height) / s) - tr.y) / size.height
      ) - y0;
...
Graph.prototype.createLayersDialog = function (onchange, inverted) {
  ...
  for (var i = 0; i < childCount; i++) {
    mxUtils.bind(this, function (layer) {
      ...
      span.setAttribute(
        "title",
        mxResources.get(model.isVisible(layer) ? "hideIt" : "show", [title])
      );
...
Graph.prototype.updateShapes = function (source, targets, replaceStyles) {
  ...
  try {
    ...
    var style = replaceStyles
      ? this.stylesheet.getCellStyle(sourceStyle, {}, false)
      : null;
    ...
    if (style != null && style[mxConstants.STYLE_SHAPE] == null) {
      if (this.model.isVertex(source)) {
        style[mxConstants.STYLE_SHAPE] =
          this.stylesheet.getDefaultVertexStyle()[mxConstants.STYLE_SHAPE];
      } else if (this.model.isEdge(source)) {
        style[mxConstants.STYLE_SHAPE] =
          this.stylesheet.getDefaultEdgeStyle()[mxConstants.STYLE_SHAPE];
      }
    }
    ...
    for (var i = 0; i < targets.length; i++) {
      if (
        (this.model.isVertex(source) && this.model.isVertex(targets[i])) ||
      ...
      if (
        mxUtils.getValue(
          this.getCellStyle(targets[i], false),
          "composite",
          "0"
        ) == "1"
      ) {
...
Graph.prototype.connectVertex = function (
  ...
  var rect = !ignoreCellAt
    ? new mxRectangle(dx + pt.x * s, dy + pt.y * s).grow(40 * s)
    : null;
  var tempCells =
    rect != null
      ? this.getCells(0, 0, 0, 0, null, null, rect, null, true)
      : null;
  ...
  if (tempCells != null) {
    ...
    for (var i = 0; i < tempCells.length; i++) {
      if (
        !this.isCellLocked(tempCells[i]) &&
        !this.model.isEdge(tempCells[i]) &&
        tempCells[i] != source
      ) {
        // Direct parent overrides all possible containers
        if (
          !this.model.isAncestor(source, tempCells[i]) &&
          this.isContainer(tempCells[i]) &&
        ...
        // Containers are used as target cells but swimlanes are used as parents
          ...
          target == null &&
          this.isCellConnectable(tempCells[i]) &&
  ...
  var duplicate =
    !mxEvent.isShiftDown(evt) || mxEvent.isControlDown(evt) || forceClone;
    ...
    if (
      ...
      try {
        if (realTarget == null && duplicate) {
          ...
          var cellToClone = this.getAbsoluteParent(
            targetCell != null ? targetCell : source
          );
 ...
 */
Graph.prototype.replaceDefaultColor = function (
  style,
  key,
  value,
  inverseValue
 ...
 * Replaces the colors for the given key.
 */
Graph.prototype.getDefaultColor = function (
  style,
  key,
  defaultValue,
  inverseDefaultValue
...
Graph.prototype.fitPages = function (pageCount, ignoreHeight) {
  ...
  var scale =
    Math.floor(
      20 * (ignoreHeight ? sx : Math.min(sx, ch / (vcount * fmt.height) / ps))
    ) / 20;
  ...
  if (mxUtils.isNode(cell.value)) {
    ...
    if (tip == null) {
      ...
      for (var i = 0; i < attrs.length; i++) {
        if (
          ((Graph.translateDiagram && attrs[i].nodeName == "label") ||
      ...
      for (var i = 0; i < temp.length; i++) {
        if (temp[i].name != "link" || !this.isCustomLink(temp[i].value)) {
          tip +=
            (temp[i].name != "link"
              ? "<b>" + mxUtils.htmlEntities(temp[i].name) + ":</b> "
              : "") +
      ...
      if (tip.length > 0) {
        ...
        if (mxClient.IS_SVG) {
          tip =
            '<div style="max-width:360px;text-overflow:ellipsis;overflow:hidden;">' +
            tip +
            "</div>";
        }
      }
...
Graph.prototype.addFlowAnimationToNode = function (node, style, scale, id) {
  if (node != null && id != null) {
    ...
    if (dashArray == "" || dashArray == null) {
      tokens = String(
        mxUtils.getValue(style, mxConstants.STYLE_DASH_PATTERN, "8")
      ).split(" ");
      var sw =
        mxUtils.getValue(style, mxConstants.STYLE_FIX_DASH, false) == 1 ||
    ...
    if (tokens.length > 0) {
      ...
      var d = Math.round(
        (sum / scale / 16) *
          parseInt(mxUtils.getValue(style, "flowAnimationDuration", 500))
      );
      ...
      node.style.animation =
        ...
        d +
        "ms " +
        mxUtils.htmlEntities(tf) +
        " infinite " +
        mxUtils.htmlEntities(ad);
...
Graph.prototype.createCrossFunctionalSwimlane = function (
  ...
  tableStyle =
    tableStyle != null
      ...
      : "shape=table;childLayout=tableLayout;" +
        (title == null ? "startSize=0;fillColor=none;" : "startSize=40;") +
        s;
  rowStyle =
    rowStyle != null
      ? rowStyle
  ...
  firstCellStyle =
    firstCellStyle != null
      ? firstCellStyle
      : "swimlane;swimlaneHead=0;swimlaneBody=0;fontStyle=0;strokeColor=inherit;" +
        "connectable=0;fillColor=none;startSize=40;" +
        s;
  cellStyle =
    cellStyle != null
      ? cellStyle
      : "swimlane;swimlaneHead=0;swimlaneBody=0;fontStyle=0;connectable=0;strokeColor=inherit;" +
        "fillColor=none;startSize=0;" +
        s;
...
Graph.prototype.visitTableCells = function (cell, visitor) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    for (var j = 0; j < cols.length; j++) {
      ...
      var head =
        mxUtils.getValue(rowStyle, mxConstants.STYLE_SWIMLANE_HEAD, 1) == 1 &&
        mxUtils.getValue(
          rowStyle,
          mxConstants.STYLE_STROKECOLOR,
          mxConstants.NONE
...
TableLayout.prototype.getRowLayout = function (row, width) {
  ...
  for (var i = 0; i < cells.length; i++) {
    ...
    if (geo != null) {
      x +=
        ((geo.alternateBounds != null ? geo.alternateBounds.width : geo.width) *
          rw) /
        sw;
    ...
    try {
      ...
      if (th > 0 && tw > 0 && rows.length > 0 && sh > 0) {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          sw = Math.max(
            sw,
            this.layoutRow(rows[i], pos, row.height, tw, lastCells)
          );
        }
          ...
          mxUtils.bind(this, function (iter) {
            ...
            if (iter.actual.cell != iter.cell) {
              if (iter.actual.row == iter.row) {
                var g =
                  iter.geo.alternateBounds != null
                    ? iter.geo.alternateBounds
                    : iter.geo;
...
(function () {
  ...
  mxShape.prototype.getFlowAnimationPath = function () {
    var paths =
      this.node != null ? this.node.getElementsByTagName("path") : null;
            ...
            if (
              ...
              for (var j = 0; j < pts2.length - 1; j++) {
                ...
                if (
                  ...
                  (pl == null ||
                    mxUtils.ptLineDist(p0.x, p0.y, p1.x, p1.y, pl.x, pl.y) >
                      thresh ||
                    mxUtils.ptLineDist(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y) >
                      thresh) &&
                  // Removes jumps on overlapping outgoing segments
                  (pn == null ||
                    mxUtils.ptLineDist(p0.x, p0.y, p1.x, p1.y, pn.x, pn.y) >
                      thresh ||
                    mxUtils.ptLineDist(p0.x, p0.y, p1.x, p1.y, p3.x, p3.y) >
                      thresh)
                ) {
                  var dx = pt.x - p0.x;
                  var dy = pt.y - p0.y;
  ...
  mxConnector.prototype.paintLine = function (ento, absPts, rounded) {
        ...
        if (last != null && rpt.type == 1) {
          ...
          if (dist > size * size && len > 0) {
            ...
            if (dist > size * size) {
              ...
              var p0Dot = new mxPoint(pt.x - n.x / 2, pt.y - n.y / 2);
              var p1Dot = new mxPoint(pt.x + n.x / 2, pt.y + n.y / 2);
              pts.push(p0);

              this.addPoints(ento, pts, rounded, arcSize, false, null, moveTo);
              ...
              if (style == "sharp") {
                ento.lineTo(p0.x - n.y * f, p0.y + n.x * f);
                ento.lineTo(p1.x - n.y * f, p1.y + n.x * f);
                ento.lineTo(p1.x, p1.y);
              } else if (style == "line") {
                ento.moveTo(p0.x + n.y * f, p0.y - n.x * f);
                ento.lineTo(p0.x - n.y * f, p0.y + n.x * f);
                ento.moveTo(p1.x - n.y * f, p1.y + n.x * f);
                ento.lineTo(p1.x + n.y * f, p1.y - n.x * f);
   ...
   */
  var mxGraphViewGetFixedTerminalPoint =
    mxGraphView.prototype.getFixedTerminalPoint;
  ...
  mxGraphView.prototype.getFixedTerminalPoint = function (
    ...
    if (
      terminal != null &&
      terminal.style[mxConstants.STYLE_PERIMETER] == "centerPerimeter"
    ) {
...
if (typeof mxVertexHandler !== "undefined") {
  (function () {
    ...
    mxWindow.prototype.closeImage = Graph.createSvgImage(
      18,
      10,
      '<path d="M 5 1 L 13 9 M 13 1 L 5 9" stroke="#707070" stroke-width="2"/>'
    ).src;
    mxWindow.prototype.minimizeImage = Graph.createSvgImage(
      14,
      10,
      '<path d="M 3 7 L 7 3 L 11 7" stroke="#707070" stroke-width="2" fill="none"/>'
    ).src;
    mxWindow.prototype.normalizeImage = Graph.createSvgImage(
      14,
      10,
      '<path d="M 3 3 L 7 7 L 11 3" stroke="#707070" stroke-width="2" fill="none"/>'
    ).src;
    mxWindow.prototype.resizeImage = Graph.createSvgImage(
      10,
      10,
      '<path d="Z" stroke="#C0C0C0" stroke-width="1" fill="none"/>'
    ).src;
    ...
    mxGraphLayout.prototype.isEdgeIgnored = function (edge) {
      return (
        graphLayoutIsEdgeIgnored.apply(this, arguments) ||
    ...
    Graph.prototype.splitEdge = function (
      ...
      this.model.beginUpdate();
      try {
        var newEdge = graphSplitEdge.apply(this, [
          edge,
          cells,
          newEdge,
          dx,
    ...
    Graph.prototype.swapShapes = function (source, target) {
      if (
        this.model.isVertex(source) &&
        this.model.isVertex(target) &&
            ...
            if (geo != null) {
              ...
              if (state != null) {
                ...
                this.setCellStyles(
                  ...
                  dirs[
                    mxUtils.mod(
                      mxUtils.indexOf(dirs, dir) + (backwards ? -1 : 1),
                      dirs.length
                    )
                  ],
                  [cell]
    ...
    Graph.prototype.isTargetShape = function (target, cells, evt) {
      ...
      for (var i = 0; i < cells.length; i++) {
        var shapes = mxUtils
          .getValue(this.getCurrentCellStyle(cells[i]), "targetShapes", "")
          .split(",");
    ...
    Graph.prototype.addText = function (x, y, state) {
      ...
      if (state != null && this.model.isEdge(state.cell)) {
        label.style =
          "edgeLabel;" + this.appendFontSize(style, this.edgeFontSize);
      ...
      var mouseListener = {
        ...
        updateCurrentState: function (me) {
          ...
          if (tmp == null || graph.getLinkForCell(tmp.cell) == null) {
            ...
            tmp =
              tmp != null && !graph.model.isAncestor(cell, tmp.cell)
                ? null
            ...
            if (this.currentLink != null) {
              ...
              if (
                ...
                beforeClick(
                  evt,
                  this.currentLink,
                  this.currentState != null ? this.currentState.cell : null
                );
              }
    ...
    Graph.prototype.duplicateCells = function (cells, append) {
      ...
      try {
        ...
        this.updateCustomLinks(
          this.createCellMapping(cloneMap, lookup),
          clones,
          this
        );
    ...
    Graph.prototype.distributeCells = function (horizontal, cells, spacing) {
      ...
      if (cells != null && cells.length > 1) {
        ...
        if (vertices.length > 2) {
          ...
          if (spacing) {
            cellsSize -= horizontal
              ? vertices[0].width / 2 + vertices[vertices.length - 1].width / 2
              : vertices[0].height / 2 +
          ...
          try {
            ...
            for (var i = 1; i < vertices.length - 1; i++) {
              ...
              if (geo != null && pstate != null) {
                ...
                if (horizontal) {
                  geo.x =
                    Math.round(t0 - (spacing ? 0 : geo.width / 2)) -
                    pstate.origin.x;
                } else {
                  geo.y =
                    Math.round(t0 - (spacing ? 0 : geo.height / 2)) -
                    pstate.origin.y;
                }
    ...
    Graph.prototype.getSvg = function (
      ...
      try {
          ...
          if (str != null && this.state.fontSize > 0) {
            try {
              if (mxUtils.isNode(str)) {
                ...
                str = mxUtils.extractTextWithWhitespace(
        ...
        imgExport.getLinkForCellState = function (state, canvas) {
          var result = state.view.graph.getAbsoluteUrl(
            imgExportGetLinkForCellState.apply(this, arguments)
          );
    ...
    Graph.prototype.addForeignObjectWarning = function (canvas, root) {
      if (
        urlParams["svg-warning"] != "0" &&
        root.getElementsByTagName("foreignObject").length > 0
      ) {
    ...
    Graph.prototype.flipCells = function (cells, horizontal) {
      ...
      try {
          ...
          if (this.model.isEdge(cells[i])) {
            ...
            if (state != null) {
              this.flipEdgePoints(
                cells[i],
                horizontal,
                (horizontal ? state.getCenterX() : state.getCenterY()) /
                  this.view.scale -
            ...
            if (geo != null) {
              this.flipChildren(
                cells[i],
                horizontal,
                horizontal ? geo.getCenterX() - geo.x : geo.getCenterY() - geo.y
              );
            }
        ...
        this.toggleCellStyles(
          horizontal ? mxConstants.STYLE_FLIPH : mxConstants.STYLE_FLIPV,
          false,
          vertices
        );
      } finally {
    ...
    Graph.prototype.insertTableColumn = function (cell, before) {
      ...
      try {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          clone.value = null;
          clone.style = mxUtils.setStyle(
            mxUtils.setStyle(clone.style, "rowspan", null),
            "colspan",
            null
          );
      ...
      try {
        ...
        if (rowGeo != null) {
          for (var i = 0; i < cells.length; i++) {
            ...
            // Removes value, col/rowspan and alternate bounds
            cell.value = null;
            cell.style = mxUtils.setStyle(
              mxUtils.setStyle(cell.style, "rowspan", null),
              "colspan",
              null
            );
    ...
    mxCellEditor.prototype.getCurrentValue = function (state) {
      if (mxUtils.getValue(state.style, "html", "0") == "0") {
        ...
        if (mxUtils.getValue(state.style, "nl2Br", "1") == "1") {
          ...
          if (
            result.length > 0 &&
            (result.substring(result.length - 5) == "<br/>" ||
              result.substring(result.length - 4) == "<br>")
          ) {
            result =
              result.substring(0, result.lastIndexOf("<br")) +
              "<div><br/></div>";
          }
        } else {
          result = result.replace(/\r\n/g, "").replace(/\n/g, "");
    ...
    var connectionHandlerIsStartEvent =
      mxConnectionHandler.prototype.isStartEvent;
    ...
    var vertexHandlerIsCustomHandleVisible =
      mxVertexHandler.prototype.isCustomHandleVisible;
        ...
        function moveLine(line, dx, dy) {
          ...
          for (var i = 0; i < line.length; i++) {
            ...
            result.push(
              pt == null
                ? null
                : new mxPoint((x0 + pt.x + dx) * s, (y0 + pt.y + dy) * s)
            );
          }
        ...
        if (rows.length > 0) {
          ...
          for (var i = 0; i < rows.length; i++) {
            mxUtils.bind(this, function (index) {
              ...
              if (graph.isCellMovable(rowState.cell)) {
                ...
                var ngeo =
                  nextRow != null ? graph.getCellGeometry(nextRow.cell) : null;
                var ng =
                  ngeo != null && ngeo.alternateBounds != null
                    ? ngeo.alternateBounds
                    : ngeo;
                ...
                handle.redraw = function () {
                  if (this.shape != null) {
                    ...
                    if (this.shape.constructor == TableLineShape) {
                      ...
                      var start = graph.getActualStartSize(
                        tableState.cell,
                        true
                      );
                      this.shape.bounds.height = 1;
                      this.shape.bounds.y =
                        this.state.y + this.state.height + dy * s;
                      this.shape.bounds.x =
                        tableState.x +
                        (index == rows.length - 1 ? 0 : start.x * s);
                      this.shape.bounds.width =
                        tableState.width -
                        (index == rows.length - 1
                          ? 0
          ...
          for (var i = 0; i < cols.length; i++) {
            mxUtils.bind(this, function (index) {
              ...
              var ngeo =
                nextCol != null ? graph.getCellGeometry(nextCol) : null;
              var ng =
                ngeo != null && ngeo.alternateBounds != null
                  ? ngeo.alternateBounds
                  : ngeo;
              ...
              handle.redraw = function () {
                if (this.shape != null) {
                  ...
                  if (this.shape.constructor == TableLineShape) {
                    ...
                    this.shape.bounds.y =
                      tableState.y +
                      (index == cols.length - 1 ? 0 : start.y * s);
              ...
              handle.setPosition = function (bounds, pt, me) {
                dx = Math.max(
                  Graph.minTableColumnWidth - g.width,
    ...
    mxVertexHandler.prototype.createMoveHandles = function () {
      ...
      for (var i = 0; i < model.getChildCount(this.state.cell); i++) {
        mxUtils.bind(this, function (rowState) {
          if (
            rowState != null &&
            model.isVertex(rowState.cell) &&
            graph.isCellMovable(rowState.cell)
          ) {
            var bounds = new mxRectangle(
              0,
              0,
              this.rowHandleImage.width,
              this.rowHandleImage.height
            );
      ...
      if (edge != null) {
        ...
        this.hint.innerHTML +=
          ...
          (unit == mxConstants.POINTS
            ...
            : formatHintText(edge.length / s, unit)) +
          ")";
      }
    ...
    HoverIcons.prototype.endMainHandle = Graph.createSvgImage(
      18,
      18,
      '<circle cx="9" cy="9" r="6" stroke="#fff" fill="' +
        HoverIcons.prototype.arrowFill +
        '"/>'
    );
    ...
    mxEdgeHandler.prototype.endTerminalHandleImage =
      HoverIcons.prototype.endTerminalHandle;
    mxEdgeHandler.prototype.fixedHandleImage = HoverIcons.prototype.fixedHandle;

    mxEdgeHandler.prototype.endFixedHandleImage =
      HoverIcons.prototype.endFixedHandle;
      ...
      try {
        if (link == null && (links == null || links.length == 0)) {
          ...
          if (link != null) {
            ...
            wrapper.appendChild(
              this.graph.createLinkForHint(link, null, this.state.cell)
            );
            ...
            for (var i = 0; i < links.length; i++) {
              mxUtils.bind(this, function (currentLink, index) {
                ...
                var updateLink = mxUtils.bind(this, function (value) {
                  var tmp = document.createElement("div");
                  tmp.innerHTML = Graph.sanitizeHtml(
                    this.graph.getLabel(this.state.cell)
                  );
    ...
    var edgeHandlerCreateConstraintHandler =
      mxEdgeHandler.prototype.createConstraintHandler;
    ...
    mxVertexHandler.prototype.redrawHandles = function () {
      if (this.moveHandles != null) {
        for (var i = 0; i < this.moveHandles.length; i++) {
          if (this.moveHandles[i] != null) {
            this.moveHandles[i].bounds.x = Math.round(
              this.moveHandles[i].rowState.x +
                this.moveHandles[i].rowState.width -
                this.moveHandles[i].bounds.width / 2
            );
            this.moveHandles[i].bounds.y = Math.round(
              this.moveHandles[i].rowState.y +
                (this.moveHandles[i].rowState.height -
                  this.moveHandles[i].bounds.height) /
                  2
            );
